...
function updatePositions() {
    ...
    for (var i = 0; i < items.length; i++) {
        var phase = Math.sin((window.scrollY / 1250) + (i % 5));
        items[i].style.left = parseInt(items[i].getAttribute('data-left')) + (100 * phase) + 'px';
    }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    for (var i = 0; i < 200; i++) {
        ...
        elem.setAttribute('data-left', String((i % cols) * s));
